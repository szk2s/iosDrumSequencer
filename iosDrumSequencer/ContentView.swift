...
class DrumSequencerConductor: ObservableObject {
  ...
  func start() {
    ...
    do {
      ...
      let closedHiHatURL = Bundle.main.resourceURL?.appendingPathComponent(
      ...
      let openHiHatURL = Bundle.main.resourceURL?.appendingPathComponent(
    ...
    sequencer.tracks[0].add(
      noteNumber: 24, velocity: 80, position: Duration(beats: 0), duration: Duration(beats: 1))

    sequencer.tracks[0].add(
      noteNumber: 24, velocity: 80, position: Duration(beats: 2), duration: Duration(beats: 1))

    sequencer.tracks[1].add(
      noteNumber: 26, velocity: 80, position: Duration(beats: 2), duration: Duration(beats: 1))
    ...
    sequencer.tracks[3].add(
      noteNumber: 26, velocity: 127, position: Duration(beats: 2), duration: Duration(beats: 1))
  }
